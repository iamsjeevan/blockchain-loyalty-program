...
const authMiddleware_1 = require("../middleware/authMiddleware"); // For protecting the mint route
...
// ...
router.post('/earn-points', authMiddleware_1.authenticateUser, async (req, res) => {
    ...
    const userFromRequest = req.user; // Use 'as any' for simplicity here, or proper typing
    ...
    let amountBigInt;
    try {
        amountBigInt = BigInt(pointsToEarn);
        if (amountBigInt <= 0n) { /* ... */ }
    }
    catch (e) { /* ... */ }
    ...
    try {
        ...
        const newBalance = await (0, blockchainService_1.getCoffeeCoinBalance)(recipientAddress);
...
router.post('/record-redemption', authMiddleware_1.authenticateUser, async (req, res) => {
    // The 'authenticateUser' middleware ensures req.user exists if token is valid
    const userFromRequest = req.user; // Or use proper typing if req.user is strongly typed globally
