...
import privyClient from '../config/privy'; // Assuming privyClient is initialized here
import dotenv from 'dotenv';

dotenv.config(); // Ensure .env is loaded if not already globally
...
declare global {
  namespace Express {
    interface Request {
      verifiedPrivyDid?: string; 
      // For full user object later:
      // user?: { privyDid: string; privyUserRaw: any; wallet?: any; };
    }
  }
}
...
export const authenticateUser = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  try {
    ...
    const verifiedClaims = await privyClient.verifyAuthToken(authToken, privyAppVerificationKey);
    ...
    const privyUserObject = await privyClient.getUser(verifiedClaims.userId);
    if (!privyUserObject) {
        ...
        // Define UserWalletDisplay and AuthenticatedBackendData if not globally available in this scope
        interface UserWalletDisplay { address: string; chainId?: string | number; walletType?: string; chainType?: string; }

        let targetWalletForBackend: UserWalletDisplay | undefined = undefined;
        const expectedChainIdString = '11155111';
        const expectedChainIdCAIP = `eip155:${expectedChainIdString}`;

        const checkAndAssignWallet = (walletSource: any): boolean => {
        ...
        if (!targetWalletForBackend && privyUserObject.linkedAccounts && Array.isArray(privyUserObject.linkedAccounts)) {
            for (const account of privyUserObject.linkedAccounts) {
                if (account.type === 'wallet') {
                    if (checkAndAssignWallet(account)) break;
                }
            }
        }

        // Extend req object correctly (ensure this matches your global declaration if you split it)
        if (!(req as any).user) (req as any).user = {}; // Initialize if not present
        (req as any).user.privyDid = privyUserObject.id;
        (req as any).user.privyUserRaw = privyUserObject; // For debugging
        (req as any).user.wallet = targetWalletForBackend;

        if (!(req as any).user.wallet) {
            console.warn(`BACKEND DEBUG: User ${privyUserObject.id} - No specific Sepolia embedded wallet found to forward.`);
        } else {
            console.log(`BACKEND DEBUG: Forwarding wallet for ${privyUserObject.id}:`, (req as any).user.wallet);
        }
    }
    ...
    if (error.status) console.error('BACKEND DEBUG: Privy error status:', error.status);
    if (error.code) console.error('BACKEND DEBUG: Privy error code:', error.code);
